  ...
  categories: [
    {
      ...
      icon: '🎓',
      types: [
        { id: 'quiz', name: 'Викторина / Тест', complexity: ['simple', 'medium', 'complex'], promptTemplate: 'Создай интерактивный тест по теме "{topic}" с {questionsCount} вопросами. Формат: {quizFormat}. Включи: экран «Старт», пошаговые вопросы, мгновенную проверку и краткое объяснение, подсчёт баллов, экран результата с «Пройти снова». Стиль: {style}.' },
    ...
    },
    {
      ...
      icon: '🎮',
      types: [
        { id: 'millionaire', name: 'Кто хочет стать миллионером?', complexity: ['simple', 'medium', 'complex'], promptTemplate: 'Создай игру «Кто хочет стать миллионером?» по теме "{topic}". 15 вопросов с нарастающей сложностью. 4 варианта ответа. 3 подсказки (50:50, звонок другу, помощь зала). Система денег, сохранение прогресса. Стиль: {style}.' },
        { id: 'jeopardy', name: 'Своя Игра (Jeopardy)', complexity: ['simple', 'medium', 'complex'], promptTemplate: 'Создай игру «Своя Игра» по теме "{topic}". Категории: {categories}. Стоимость вопросов: 100-500. Таблица с выбором, модальные окна для вопросов, подсчёт очков. Стиль: {style}.' },
        { id: 'memory', name: 'Найди пару (Memory)', complexity: ['simple', 'medium', 'complex'], promptTemplate: 'Создай игру «Найди пару» по теме "{topic}". Карточек: {cardsCount}. Механика: переворот, поиск совпадений, подсчёт ходов, таймер. Добавь разные уровни сложности. Стиль: {style}.' },
        { id: 'clicker', name: 'Кликер (Idle Game)', complexity: ['simple', 'medium', 'complex'], promptTemplate: 'Создай игру-кликер по теме "{topic}". Основная валюта: {currency}. Улучшения: {upgrades}. Механика: клик → ресурс → покупка улучшений → авто-доход. Добавь достижения и сохранения. Стиль: {style}.' },
        { id: 'hangman', name: 'Виселица', complexity: ['simple', 'medium', 'complex'], promptTemplate: 'Создай игру «Виселица» по теме "{topic}". Список слов: {wordsCount} штук. Отрисовка виселицы по шагам, ввод букв, подсчёт побед/поражений. Стиль: {style}.' },
        { id: 'snake', name: 'Змейка', complexity: ['simple', 'medium', 'complex'], promptTemplate: 'Создай классическую игру «Змейка» с тематикой "{topic}". Управление стрелками, счёт, рекорд, ускорение со временем. Добавь бонусы и препятствия. Стиль: {style}.' },
        { id: 'puzzle', name: 'Пазл / Сборка', complexity: ['simple', 'medium', 'complex'], promptTemplate: 'Создай игру-пазл по теме "{topic}". Количество элементов: {piecesCount}. Механика: перетаскивание, сборка изображения/текста. Добавь таймер и подсказки. Стиль: {style}.' },
        { id: 'battle', name: 'Битва знаний (2 игрока)', complexity: ['simple', 'medium', 'complex'], promptTemplate: 'Создай игру на двоих по теме "{topic}". По очереди отвечают на вопросы, правильные ответы = очки атаки. Здоровье, раунды, определение победителя. Стиль: {style}.' },
      ],
    },
    {
      id: 'data',
    ...
    },
    {
      ...
      types: [
        ...
        { id: 'report', name: 'Отчёт о проделанной работе', complexity: ['medium'], promptTemplate: 'Создай отчёт для "{topic}". Разделы: метрики, графики, достижения, проблемы, планы. Период: {period}. Добавь кнопку «Печать / сохранить» через window.print или копирование текста, без PDF-библиотек. Стиль: {style}.' },
      ],
    },
    {
      id: 'documents',
      ...
      types: [
        ...
        { id: 'survey', name: 'Анкета / Опросник', complexity: ['medium'], promptTemplate: 'Создай анкету для "{topic}". Типы вопросов: выбор, ввод текста, шкала, множественный выбор. Валидация, прогресс-бар, экспорт ответов в текст или JSON локально (без API). Стиль: {style}.' },
      ],
    },
    {
      id: 'marketing',
      ...
      icon: '🎯',
      types: [
        { id: 'quiz_landing', name: 'Квиз-лендинг', complexity: ['complex'], promptTemplate: 'Создай квиз-лендинг для "{topic}". Вопросы: {questionsCount}. Прогресс-бар, варианты ответов, финальный экран с формой контактов (клиентская валидация, без отправки на сервер), страница благодарности. Стиль: {style}.' },
