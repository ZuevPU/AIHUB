...
const TabsList = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement>>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    role="tablist"
    className={cn('inline-flex h-10 items-center justify-center rounded-md bg-zinc-100 p-1 text-zinc-500', className)}
    {...props}
  />
));
...
>(({ className, active, ...props }, ref) => (
  <button
    ...
    className={cn(
      ...
      active ? 'bg-white text-zinc-950 shadow-sm border-b-2 border-zinc-900' : 'hover:bg-zinc-200/50 hover:text-zinc-900',
      className
    )}
    {...props}
  />
