...
import { promptBuilderVideoCategories, videoOutputLabels } from '@/data/promptBuilderVideoCategories';
...
export function PromptBuilderVideoPage() {
  ...
  const fullPrompt = useMemo(() => {
    const lines: string[] = [];
    let num = 1;
    for (const cat of promptBuilderVideoCategories) {
      const value = getValue(cat.id);
      if (value) {
        const label = videoOutputLabels[cat.id] || cat.title;
    ...
    <div className="container mx-auto px-4 py-8 md:px-6 max-w-6xl">
      ...
      <div className="mb-8">
        ...
        <p className="text-zinc-600">
          Универсальная библиотека параметров. Выберите опции или введите свой вариант — промпт соберётся автоматически
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          {promptBuilderVideoCategories.map((category) => (
            <div
              key={category.id}
            ...
            >
              ...
              <div className="flex flex-wrap gap-2 mb-3">
                {category.options.map((opt) => (
                  ...
                  >
                    {opt.text.length > 45 ? opt.text.slice(0, 45) + '…' : opt.text}
                  </button>
                ))}
              </div>
              <input
