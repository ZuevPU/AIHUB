...
import {
  promptBuilderPresentationCategories,
  presentationOutputLabels,
} from '@/data/promptBuilderPresentationCategories';
...
export function PromptBuilderPresentationPage() {
  ...
  const fullPrompt = useMemo(() => {
    const lines: string[] = [];
    let num = 1;
    for (const cat of promptBuilderPresentationCategories) {
      const value = getValue(cat.id);
      if (value) {
        const label = presentationOutputLabels[cat.id] || cat.title;
    ...
    <div className="container mx-auto px-4 py-8 md:px-6 max-w-6xl">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          {promptBuilderPresentationCategories.map((category) => (
            <div
              key={category.id}
